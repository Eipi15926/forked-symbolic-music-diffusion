...
class Melody(events_lib.SimpleEventSequence):
  ...
  def from_quantized_sequence(self,
    ...
    if mode =='part':# select particular part
      notes = sorted([n for n in quantized_sequence.notes
                    if n.part in mode_related_info and
                    n.quantized_start_step >= search_start_step],
                  key=lambda note: (note.quantized_start_step, -note.pitch))
    elif mode == 'inst':# select particular instrument
      notes = sorted([n for n in quantized_sequence.notes
                      if n.instrument in mode_related_info and
                      n.quantized_start_step >= search_start_step],
                    key=lambda note: (note.quantized_start_step, -note.pitch))
    elif mode =='pitch':# divided by pitch value
      notes = sorted([n for n in quantized_sequence.notes
                      if n.pitch in mode_related_info and
                      n.quantized_start_step >= search_start_step],
                    key=lambda note: (note.quantized_start_step, -note.pitch))
    else:
      return
    ...
    if not notes:
      return
    """
    # TODO: check max language correctness
    try:
      all_notes = quantized_sequence.notes
      my_melody_end_step_absolute = (max(all_notes, key=lambda x: x.quantized_end_step)).quantized_end_step
    except:
      print("Exception in note prepocessing.")
      return
    ...
    """
    melody_start_step = search_start_step
    for note in notes:
    ...
    length = my_melody_end_step_absolute - search_start_step
  ...
  def to_sequence(self,
    ...
    for step, note in enumerate(self):
      cur_note_end_time = step * seconds_per_step + sequence_start_time
      if cur_note_end_time > sequence_end_time:
        sequence_end_time = cur_note_end_time
    ...
    if sequence.notes:
      ...
      sequence.total_time = sequence_end_time
    return sequence
...
def extract_melodies(note_sequence, keep_longest_split=False, mode='part',mode_related_info=1):
  ...
  Args:
    ...
    mode: 
      1.'part': select a particular part according to mode_related info
      2.'inst': select a set of instruments according to mode_related info
      3.'pitch': select a range of pitch according to mode_related info
    mode_related_info:
      1.'part': when mode='part', mode_related_info is thus an interger number
      2.'inst': when mode='inst', mode_related_info is a set of instrument numbers
      3.'pitch': when mode='pitch',mode_related_info is a set of pitch numbers
  ...
  for split_ns in splits:
    ...
    try:
        melody.from_quantized_sequence(qs,
                                      ignore_polyphonic_notes=True,
                                      mode=mode,
                                      mode_related_info=mode_related_info,
                                      gap_bars=np.inf)
    except note_seq.NonIntegerStepsPerBarError:
      print("error in myns_utils.py line 610")
      continue
